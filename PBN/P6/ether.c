...
void ether_put(uint8_t c){
	uint8_t a;
	// Ens esperem mentres la cua estigui plena
...
static void gap(void){
	...
	if (mchar_next(&mchar)==MDot) timer_after(TIMER_MS(DOT),dot);
	// Si no, una ratlla
	else timer_after(TIMER_MS(DASH),dash);
}
